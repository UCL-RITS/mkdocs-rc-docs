...
ssh -o ProxyJump=somename.ucl.ac.uk grace.rc.ucl.ac.uk
```
or
```
# Copy 'my_file' from the machine you are logged in to into your Scratch on Grace
scp -o ProxyJump=somename.ucl.ac.uk my_file grace.rc.ucl.ac.uk:~/Scratch/
```
...
Host <name>
   ...
   proxyCommand ssh -W <remote_hostname>:22 <remote_user_id>@somename.ucl.ac.uk
```
...
Host myriad
   ...
   proxyCommand ssh -W myriad.rc.ucl.ac.uk:22 ccxxxxx@somenameucl.ac.uk

Host login05
   User ccxxxxx
   HostName login05.external.legion.ucl.ac.uk
   proxyCommand ssh -W login05.external.legion.ucl.ac.uk:22 ccxxxxx@somename.ucl.ac.uk

Host aristotle
   User ccxxxxx
   HostName aristotle.rc.ucl.ac.uk
   proxyCommand ssh -W aristotle.rc.ucl.ac.uk:22 ccxxxxx@somename.ucl.ac.uk
```
...
```
TODO: Waiting for instructions from RCI
```